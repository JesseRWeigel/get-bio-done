#!/usr/bin/env python3
"""Statusline hook for Claude Code — shows GBD project status.

Reads JSON from stdin (Claude Code session data), outputs ANSI-formatted
statusline with: GBD | model | phase/plan progress | context bar
...
def get_phase_info() -> str:
    ...
    # Look for .gbd directory
    cwd = os.getcwd()
    search = cwd
    while search != os.path.dirname(search):
        state_json = os.path.join(search, ".gbd", "state.json")
...
def format_statusline(data: dict) -> str:
    ...
    # Brand
    parts.append("\033[1;36mGBD\033[0m")  # bold cyan
