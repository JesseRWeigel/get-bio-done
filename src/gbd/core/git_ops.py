...
Adapted from GPD's git_ops.py for bioinformatics research.
"""
...
def check_no_large_data_files(layout: ProjectLayout, files: list[str]) -> list[str]:
    """Check that large data files (FASTQ, BAM, VCF) are not being committed."""
    violations = []
    large_extensions = {".fastq", ".fq", ".bam", ".sam", ".cram", ".vcf", ".bcf", ".bed", ".bigwig", ".bw"}
    for f in files:
        path = layout.root / f
        if path.suffix.lower() in large_extensions or f.endswith(".fastq.gz") or f.endswith(".fq.gz"):
            violations.append(f"Large data file staged (use Git LFS or .gitignore): {f}")
    return violations
...
def run_pre_commit_checks(layout: ProjectLayout, files: list[str]) -> list[str]:
    ...
    violations.extend(check_no_large_data_files(layout, files))
...
def find_partial_completion(
    ...
    Searches git log for [gbd] commits matching the phase/plan.
